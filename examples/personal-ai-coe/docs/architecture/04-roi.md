...
|---|---|---|
| scheduled loop tokens | summarise, classify, propose facts | Model gateway — economy tier (priced as Claude Haiku 4.5) input tokens; — output tokens |
| ad-hoc session tokens | operator-initiated work sessions | Model gateway — workhorse tier (priced as Claude Sonnet 5) input tokens; — output tokens |
| weekly deep review tokens | the weekly review pass over the ledger | Model gateway — frontier tier (priced as Claude Opus 5) input tokens; — output tokens |
| off-machine backup | backup | Object storage (priced as Cloudflare R2) — standard storage |

...
|---|---|---|
| answer tokens | answer with citations | Model gateway — workhorse tier (priced as Claude Sonnet 5) input tokens; — output tokens |
| scope-check and rerank tokens | scope check, rerank | Model gateway — economy tier (priced as Claude Haiku 4.5) input tokens; — output tokens |
| vector store reads | retrieve clauses | Managed vector store (priced as Pinecone serverless) — read units |
| vector store writes and storage | embed, swap index | Managed vector store (priced as Pinecone serverless) — write units; — storage |
| request compute | the whole question path | Serverless function (priced as AWS Lambda) — compute duration; — invocations |
| clause text and audit records | audit record | Object storage (priced as Cloudflare R2) — standard storage |
| re-index orchestration | monthly re-index | Durable workflow runtime (priced as AWS Step Functions) — state transitions |
...
Object storage (priced as Cloudflare R2): 40 GB (`fx.archive.gb`) × $0.015 per GB per month = **$0.60**

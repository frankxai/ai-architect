...
|---|---|---|
| model input and output tokens | classify and route, draft reply | Model gateway — workhorse tier (priced as Claude Sonnet 5) input tokens; — output tokens |
| vector store reads | retrieve macros | Managed vector store (priced as Pinecone serverless) — read units |
| vector store writes and storage | embed changed macros, swap index | Managed vector store (priced as Pinecone serverless) — write units; — storage |
| worker compute | all worker stages | Serverless function (priced as AWS Lambda) — compute duration; — invocations |
| workflow state transitions | the triage job's steps and retries | Durable workflow runtime (priced as AWS Step Functions) — state transitions |
